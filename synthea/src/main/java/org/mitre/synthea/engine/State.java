...
public abstract class State implements Cloneable, Serializable {
  ...
  public static State build(Module module, String name, JsonObject definition) throws Exception {
    ...
    if (type.equals("Physiology")) {
      // The Physiology feature has been removed from the build due to unstable dependencies.
      // Fall back to the mandatory 'alt_direct_transition'.
      State simple = new Simple();
      simple.name = name;
      simple.module = module;
      JsonObject simpleDef = new JsonObject();
      simpleDef.add("direct_transition", definition.get("alt_direct_transition"));
      simple.initialize(module, name, simpleDef);
      return simple;
    }

    String className = State.class.getName() + "$" + type;
  ...
  public static class Physiology extends State {
    ...
    // Physiology feature disabled. The fields below are commented out to allow compilation.
    ...
    @Override
    public boolean process(Person person, long time) {
      // Physiology feature disabled. This state will be skipped by the modified `build` method.
      return true;
    }
